...
const WEBHOOK_URL = null; // Désactivé pour GitHub
...
avisForm.onsubmit = async function(e) {
    ...
    // Si pas de webhook configuré, affiche un message
    if (!WEBHOOK_URL) {
        avisList.innerHTML = `<div style="color:#fbbf24;margin-top:1em;">⚠️ Les avis sont temporairement désactivés.<br>Rejoins notre Discord pour donner ton avis!</div>`;
        return;
    }
...
// Animation sur les box au clic - Version simplifiée
document.querySelectorAll('.download-box').forEach(box => {
    ...
    // SUPPRIMÉ: Les événements mouseenter et mouseleave qui causaient le bug
});

// Animation sur les boutons au clic - Version simplifiée
...
const DISCORD_CLIENT_ID = window.APP_CONFIG?.DISCORD_CLIENT_ID || "1406428707681472612";
const REDIRECT_URI = window.APP_CONFIG?.REDIRECT_URI || window.location.origin + "/";
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Charger les données des cheats depuis le localStorage
    loadCheatBoxes();
...
// Fonction pour charger les box depuis les données sauvegardées
function loadCheatBoxes() {
    const savedData = localStorage.getItem('cheats_data');
    if (savedData) {
        const cheatsData = JSON.parse(savedData);
        const cheatList = document.getElementById('cheat-list');
        
        // Vider la liste actuelle
        cheatList.innerHTML = '';
        
        // Créer les nouvelles box
        cheatsData.forEach((cheat, index) => {
            const box = document.createElement('div');
            box.className = `download-box fortnite-box ${!cheat.status ? 'down' : ''}`;
            box.style.animationDelay = `${index * 0.1}s`; // Ajoute le délai d'animation
            ...
            img.onerror = function() {
                // Si l'image ne charge pas, utilise un placeholder
                this.src = 'images/down-placeholder.png';
                this.onerror = null; // Évite la boucle infinie
            };
            
            box.innerHTML = `
                <div class="box-title">${cheat.title}</div>
                <div class="box-price-stock">
                    <span class="box-price">Price: <b>Free</b></span>
                    <span class="box-stock">${cheat.status ? 'Update' : 'DOWN'}</span>
                </div>
                <a href="${cheat.link}" class="download-btn" target="_blank">Télécharger</a>
            `;
            
            // Insère l'image en premier
            box.insertBefore(img, box.firstChild);
            
            cheatList.appendChild(box);
        });
        
        // Réappliquer les événements sur les nouvelles box
        setupBoxEvents();
    }
}

// Fonction pour configurer les événements sur les box
function setupBoxEvents() {
    // Animation sur les box au clic uniquement
    document.querySelectorAll('.download-box').forEach(box => {
        box.addEventListener('mousedown', () => {
            box.style.transform = 'scale(0.97)';
            setTimeout(() => {
        ...
        // SUPPRIMÉ: Les événements mouseenter et mouseleave
    });
...
// Corrige le canvas pour ne pas bloquer les interactions
const canvasElement = document.getElementById('stars-bg');
if (canvasElement) {
